...
const DefaultAccess: React.FC<{ show: boolean; onHide: () => void }> = ({ show, onHide }) => {
  const [menus, setMenus] = useState<MenuItem[]>(initialMenus)


const handleCheck = (menuId: string, type: 'menu' | 'function' | 'submenu', itemId?: string) => {
    setMenus(prevMenus => {
        ...
        if (menu.id === menuId) {
          ...
          if (type === 'menu') {
            ...
            newMenu = {
              ...newMenu,
              checked: newChecked,
              functions: newMenu.functions.map(f => ({...f, checked: newChecked})),
              subMenus: newMenu.subMenus.map(s => ({...s, checked: newChecked})),
            };
          } else if (type === 'function' || type === 'submenu') {
            const targetArray = type === 'function' ? newMenu.functions : newMenu.subMenus;
            ...
            if (type === 'function') {
              ...
              if (updatedArray.some(item => item.checked)) {
                newMenu.functions = newMenu.functions.map(f => ({...f, checked: true}));
              } else {
                // Si tous les sous-menus sont décochés, décocher toutes les fonctions
                newMenu.functions = newMenu.functions.map(f => ({...f, checked: false}));
              }
            }
      ...
      {menus.map(menu => (
        <div key={menu.id} className="mb-6">
          <div className="form-check form-switch form-check-custom form-check-solid">
            ...
            />
            <label className="form-check-label fw-bold fs-5">{menu.name}</label>
          </div>
          <div className="ms-20 mt-3">
            <div className="d-flex flex-column">
              <div className="mb-3">
                {menu.functions.map(func => (
                  <div key={func.id} className="form-check form-switch form-check-custom form-check-solid me-15 d-inline-block">
                    <input
                      className="form-check-input"
                      type="checkbox"
                      checked={func.checked}
                      onChange={() => handleCheck(menu.id, 'function', func.id)}
                    />
              ...
              <div className="ms-20 d-flex">
                {menu.subMenus.map(subMenu => (
                  <div key={subMenu.id} className="form-check form-switch form-check-custom form-check-solid me-8">
                    <input
                      className="form-check-input"
                      type="checkbox"
                      checked={subMenu.checked}
                      onChange={() => handleCheck(menu.id, 'submenu', subMenu.id)}
                    />
